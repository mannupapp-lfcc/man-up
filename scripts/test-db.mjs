...
// database has not recorded yet, run the test file, ROLLBACK. Nothing a test or a
...
try {
  ...
  console.error(
    "No migration history on this database. Run:\n" +
      ...
      "for each migration already applied by hand.",
  );
  process.exit(1);
}

const pending = sqlFiles(MIGRATIONS_DIR).filter((f) => !applied.has(basename(f).split("_")[0]));
const tests = process.argv.length > 2 ? process.argv.slice(2) : sqlFiles(TESTS_DIR);
